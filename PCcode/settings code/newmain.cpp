...
const set<string> GVal={
    ...
    "Up_arrow","Down_arrow", "OnScreenKeyboard", "N/A", "Lclick", "Rclick","backsideclick","frontsideclick","Delete"
};
...
void createDefault(){
    ...
    if(!file.is_open()){
        ...
        file<<"For gesture settings there can be multiple key presses that are pressed in order. if you type a phrase in \"\" then they will be typed out. if you type a character (ex: \"a\") then it will be held.\nAproved macros:\nLclick, Rclick, backsideclick, frontsideclick, onScreenKeyboard, Space, Alt, Win, Ctrl, Lshift, Caps, Tab, Esc, Enter, F1-F12, Volume_up, Volume_down, Mute, Pause/Play, Backspace, Delete, Left_arrow, Right_arrow, Up_arrow, Down_arrow, any other expected values in \"\".\nexample formats:\nShift + TabCtrl + \"s\"Ctrl + \"t\" + \"youtube.com\" + Enter\nNumerical settings require a number (integer)\n\n";
        file<<"Gesture settings (type \"N/A\" for no action):\n";
        for(const auto& pair :GDsettings){
            file<<pair.first<<": "<< pair.second<< "\n";
        }
        file<<"Numerical settings (Requires a value):\n";
